...
void CPicking::Tick()
{
	...
	XMMatrixInverse(nullptr, ProjMatrixInv);
	XMVector3TransformCoord(vecMousePos, ProjMatrixInv);

	/* 뷰스페이스 상의 마우스 레이와 레이포스를 구하자. */
	_vector vecRayPos = XMVectorSet(0.f, 0.f, 0.f, 0.f);
	_vector vecRayDir = vecRayPos = vecMousePos;
	

	/* 월드스페이스 상의 마우스 레이와 레이포스를 구하자. */
	_matrix		ViewMatrixInv;
	ViewMatrixInv = pGameInstance->Get_TransformMatrix(CPipeLine::D3DTS_VIEW);
	XMMatrixInverse(nullptr, ViewMatrixInv);
	XMVector3TransformCoord(vecRayPos, ViewMatrixInv);
	XMVector3TransformNormal(vecRayDir, ViewMatrixInv);
	XMVector3Normalize(vecRayDir);
...
_bool CPicking::Intersect_InLocalSpace(_vector vPointA, _vector vPointB, _vector vPointC, _float3* pOut)
{
	...
	*/
	...
	_vector vLocalRayPos = XMVectorSet(0.f, 0.f, 0.f, 1.f);
	vLocalRayPos = XMLoadFloat3(&m_vRayPos_Local);
	_vector vLocalRayDir = XMLoadFloat3(&m_vRayDir_Local);
